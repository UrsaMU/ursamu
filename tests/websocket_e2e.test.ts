...
Deno.test("WS — typed { type:'cmd', data } message is treated as a command", OPTS, async () => {
  ...
  assertEquals(logs.filter((w) => w.includes("ws:rate-limited")).length, 2);
...
Deno.test("WS — rate limit resets after window expires", OPTS, async () => {
  ...
  const warnsAfterFirst = logs.filter((w) => w.includes("ws:rate-limited")).length;
  ...
  const warnsAfterSecond = logs.filter((w) => w.includes("ws:rate-limited")).length;
  assertEquals(warnsAfterSecond, 2); // no new logs
