...
export default () =>
  addCmd({
    ...
    exec: async (ctx, args) => {
      ...
      let output = center(
        `%cy[%cn %ch+FINGER%cn ${displayName(en, tar)} %cy]%cn`,
        78,
        "%cr=%cn",
      ) + "\n";
      const exclude = [
      ...
      if (await isAdmin(en)) {
        ...
        const lastLogin = tar.flags.includes("connected")
          ? "ONLINE"
          : new Date(tar.data?.lastLogin).toLocaleDateString("en-US");
      ...
      for (const attr of exclude) {
        ...
        if (stat) {
          output += `%ch${
            capString(stat.name.slice(7).replace("_", " "))
          }: %cn${stat.value}\n`;
        }
      }
      ...
      for (const attr of tar.attributes) {
        if (attr.name.startsWith("finger.") && !exclude.includes(attr.name)) {
          output += `%ch${
            capString(attr.name.slice(7).replace("_", " "))
          }: %cn${attr.value}\n`;
        }
      }
      output += repeatString("%cr=%cn", 78) + "\n";
