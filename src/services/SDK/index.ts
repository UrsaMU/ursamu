...
import { dbojs, chans, texts, bboards } from "../Database/index.ts";
  ...
  const u: IUrsamuSDK = {
    ...
    bb: {
      listBoards: async () => {
        ...
        return boards.map((b: unknown) => {
          const bd = b as Record<string, unknown>;
          return {
            id: String(bd.id || ""),
            name: String(bd.name || ""),
            description: bd.description ? String(bd.description) : undefined,
            order: Number(bd.order || 0),
            postCount: Number(bd.postCount || 0),
            newCount: 0,
          };
        });
      },
      listPosts: async (_boardId: string) => [],
      readPost: async (_boardId: string, _postNum: number) => null,
      post: async (_boardId: string, _subject: string, _body: string) => ({ id: "" }),
      editPost: async () => {},
      deletePost: async () => {},
      createBoard: async (name: string) => ({ id: "", name }),
      destroyBoard: async () => {},
      markRead: async () => {},
      newPostCount: async () => 0,
      totalNewCount: async () => 0,
    },
