 ...
 * Player @name enforces unique login names via isPlayerNameTaken.
 */
...
async function setAttr(
  ...
  u.send(
    `${attrKey.toUpperCase()} set on ` +
      `${u.util.displayName(tar, u.me)}.`,
  );
}
...
EXAMPLES
  ...
  exec: (u) =>
    setAttr(u, "description", "@desc <target>=<description>"),
});
...
EXAMPLES
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (!parentStr) {
      ...
      u.send(
        `Parent cleared on ${u.util.displayName(tar, u.me)}.`,
      );
      return;
    }
    const parent = await u.util.target(u.me, parentStr);
    if (!parent) {
      u.send(`I can't find parent '${parentStr}'.`);
      return;
    }
    ...
    u.send(
      `Parent of ${u.util.displayName(tar, u.me)} set to ` +
        `${u.util.displayName(parent, u.me)}.`,
    );
  },
});
...
Plain moniker text must not match another player's login name.
...
EXAMPLES
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (eq === -1) {
      u.send("Usage: @moniker <target>=<moniker>");
      return;
    }
    ...
    if (!stripped) {
      u.send("Moniker cannot be empty.");
      return;
    }
    const tar = await u.util.target(u.me, targetStr);
    if (!tar) {
    ...
    await u.db.modify(tar.id, "$set", {
      "data.moniker": moniker.trim(),
    });
    u.send(
      `Set moniker for ${tar.name} to ${moniker.trim()}.`,
    );
  },
});
...
function addFormatCmd(cmdName: string, attrName: string): void {
  addCmd({
    ...
    category: "Building",
    help:
      `${cmdName} <target>[=<format>]  — Set or clear ${attrName} ` +
      `on an object.
...
EXAMPLES
  ...
  ${cmdName} here=`,
    exec: async (u: IUrsamuSDK) => {
      const targetStr = u.util.stripSubs(u.cmd.args[0] ?? "")
        .trim();
      const format = u.cmd.args[1] ?? "";
      const tar = await u.util.target(u.me, targetStr);
      if (!tar) {
        u.send("%chGame>%cn Target not found.");
        return;
      }
      if (!(await u.canEdit(u.me, tar))) {
        u.send("%chGame>%cn Permission denied.");
        return;
      }
      const attrs = (
        tar.state.attributes as
          | Array<{ name: string }>
          | undefined ?? []
      ...
      if (format) {
        ...
        u.send(
          `%chGame>%cn Cleared ${cmdName} on ${tar.name}.`,
        );
      }
    },
  });
