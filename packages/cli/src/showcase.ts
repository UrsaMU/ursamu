...
export type ShowcaseStep =
  | { sub: string; label?: string }
  | { note: string; label?: string }
  | { reset: boolean; label?: string }
  | { emit: string; label?: string }
  | { expect: string; label?: string }
  | ShowcaseStepCmd
  | ShowcaseStepEval;
...
export async function discoverShowcases(root: string, filterPath?: string): Promise<ShowcaseFile[]> {
  const files: ShowcaseFile[] = [];
  const globs = [
    join(root, "packages", "*", "showcases", "*.json"),
    join(root, "src", "plugins", "*", "showcases", "*.json"),
    join(root, "showcases", "*.json"),
  ];
  for (const g of globs) {
    for await (const entry of expandGlob(g)) {
      if (filterPath && !entry.path.startsWith(filterPath)) {
        continue;
      }
      try {
...
Examples:
  ...
  const scriptDir = import.meta.url.startsWith("file://")
    ...
    : Deno.cwd();
  const root = dirname(dirname(dirname(scriptDir))); // packages/cli/src → packages/cli → packages → engine root

  const cwd = Deno.env.get("INIT_CWD") || Deno.env.get("PWD") || Deno.cwd();
  const isSubdirectoryOfShowcases = cwd.includes("/packages/") || cwd.includes("/src/plugins/") || cwd.endsWith("/showcases") || cwd.includes("/showcases/");
  const filterPath = isSubdirectoryOfShowcases ? cwd : undefined;
  const all = await discoverShowcases(root, filterPath);
