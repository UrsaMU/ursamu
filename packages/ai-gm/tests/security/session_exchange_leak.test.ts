...
const TEST_SECRET = "test-session-leak-secret";

function makeReq(path: string, secret = TEST_SECRET): Request {
...
function withSecret<T>(fn: () => Promise<T>): Promise<T> {
  const prev = Deno.env.get("GM_API_SECRET");
  const prevOpen = Deno.env.get("GM_API_OPEN");
  Deno.env.set("GM_API_SECRET", TEST_SECRET);
  Deno.env.delete("GM_API_OPEN");
  return fn().finally(() => {
    if (prev !== undefined) Deno.env.set("GM_API_SECRET", prev);
...
Deno.test({
  name: "SECURITY: GET /api/gm/sessions/:id must not return exchanges from unrelated sessions",
  sanitizeResources: false,
  sanitizeOps: false,
  fn: () =>
    withSecret(async () => {
