...
import { getDraft, setDraft } from "./draft.ts";

/** Start a new draft: @mail <target>=<subject> */
export async function mailDraftNew(
  u: IUrsamuSDK,
  rawinput: string,
): Promise<void> {
  if (getDraft(u.me)) {
    u.send(
      "%chMAIL:%cn You already have a draft in progress. " +
        "Use '@mail/abort' to discard it.",
    );
    return;
  }
  ...
  if (!target) {
    u.send(`%chMAIL:%cn Target '${targetName}' not found.`);
    return;
  }
  ...
  u.send(
    `%chMAIL:%cn Draft started to %ch${target.name}%cn. ` +
      `Subject: ${draft.subject}`,
  );
  u.send(
    "Use '-<text>' to add lines. Use '@mail/send' to send " +
      "or '@mail/abort' to cancel.",
  );
}

/** Append a line to the active draft body. Used by the '-' command. */
export async function mailAppend(
  u: IUrsamuSDK,
  text: string,
): Promise<void> {
  const draft = getDraft(u.me);
  if (!draft) {
    u.send(
      "%chMAIL:%cn No draft in progress. Use " +
        "'@mail <target>=<subject>' to start one.",
    );
    return;
  }
...
export async function mailSubject(
  ...
  const draft = getDraft(u.me);
  if (!draft) {
    u.send("%chMAIL:%cn No draft in progress.");
    return;
  }
  const subject = subArgs.trim();
  if (!subject) {
    u.send("%chMAIL:%cn Usage: @mail/subject <text>");
    return;
  }
...
export async function mailCC(
  ...
  if (!target) {
    u.send(
      `%chMAIL:%cn Target '${subArgs.trim()}' not found.`,
    );
    return;
  }
  draft.cc ??= [];
...
export async function mailAttach(
  u: IUrsamuSDK,
  subArgs: string,
): Promise<void> {
  const draft = getDraft(u.me);
...
export async function mailProof(u: IUrsamuSDK): Promise<void> {
  ...
  if (m.bcc?.length) {
    u.send(`BCC:     ${await resolveNames(m.bcc)}`);
  }
  if (m.attachments?.length) {
    u.send(`Attach:  ${m.attachments.join(", ")}`);
  }
...
/** Send the active draft. Enforces quota; emits mail:received. */
export async function mailSend(u: IUrsamuSDK): Promise<void> {
  ...
  if (!draft.message?.trim()) {
    u.send(
      "%chMAIL:%cn Cannot send a message with no body. " +
  ...
  for (const ref of (draft.to ?? [])) {
    ...
    if (count >= MAIL_QUOTA) {
      const obj = await u.util.target(u.me, id, true)
        .catch(() => null);
      u.send(
        `%chMAIL:%cn Skipping ${obj?.name ?? ref} — ` +
          `mailbox full (${MAIL_QUOTA} messages).`,
      );
    } else {
      filteredTo.push(ref);
    }
  ...
  if (filteredTo.length === 0) {
    u.send(
      "%chMAIL:%cn All recipients' mailboxes are full. " +
        "Message not sent.",
    );
    return;
  }
  ...
  for (const ref of allTo) {
    ...
    if (id !== u.me.id) {
      u.send(
        `%chMAIL:%cn You have a new message from ` +
          `${u.me.name ?? "Unknown"}.`,
        id,
      );
...
export async function mailReply(
  ...
  if (getDraft(u.me)) {
    u.send(
      "%chMAIL:%cn Draft in progress. Use '@mail/abort' " +
        "to discard it first.",
    );
    return;
  }
  const num = parseInt(subArgs.trim() || "");
  const mails = (await getMyMail(u.me.id, "inbox"))
    .sort((a, b) => a.date - b.date);
  if (!num || num < 1 || num > mails.length) {
    u.send(
      `%chMAIL:%cn Usage: @mail/${
        replyAll ? "replyall" : "reply"
      } <message number>`,
    );
    return;
  }
  ...
  const toList = replyAll
    ? [...new Set(
      [orig.from, ...orig.to].filter((r) => r !== `#${u.me.id}`),
    )]
    : [orig.from];
  ...
  if (orig.id) {
    await mailDb.modify(
      { id: orig.id },
      "$set",
      { replied: true } as Partial<IMail>,
    ).catch(() => null);
  }
  const label = replyAll
    ? `Replying to all (${toList.length} recipients)`
    : `Replying to "${orig.subject}"`;
  u.send(
    `%chMAIL:%cn ${label}. Use '-<text>' to add lines, ` +
      `'@mail/send' to send.`,
  );
}
...
export async function mailForward(
  ...
  if (eqIdx === -1) {
    u.send(
      "%chMAIL:%cn Usage: @mail/forward " +
        "<message number>=<target>",
    );
    return;
  }
  ...
  const fwd: IMail = {
    ...
    from: `#${u.me.id}`,
    to: [`#${target.id}`],
    subject: orig.subject.startsWith("Fwd: ")
      ? orig.subject
      : `Fwd: ${orig.subject}`,
    message:
      `---------- Forwarded Message ----------\n${orig.message}`,
  ...
  if (orig.id) {
    await mailDb.modify(
      { id: orig.id },
      "$set",
      { forwarded: true } as Partial<IMail>,
    ).catch(() => null);
  }
  u.send(`%chMAIL:%cn Message forwarded to ${target.name}.`);
  u.send(
    `%chMAIL:%cn You have a forwarded message from ` +
      `${u.me.name ?? "Unknown"}.`,
    target.id,
  );
}
