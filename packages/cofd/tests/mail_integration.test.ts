import { assertEquals, assertStringIncludes } from "@std/assert";
import { describe, it } from "@std/testing/bdd";
import { mailDb } from "@ursamu/mail";
import { mockU, mockPlayer } from "./helpers/mockU.ts";
import { approveExec, denyExec } from "../src/commands/approve.ts";
import { sendCofdMail } from "../src/integrations/mail.ts";
import { jobs, type IJob } from "@ursamu/jobs";
import type { CofdCgState } from "../src/chargen/index.ts";
import { dbojs } from "@ursamu/mush";

const OPTS = { sanitizeResources: false, sanitizeOps: false };

function fakeCgState(num: number): CofdCgState {
  return {
    stage: 6,
    sheet: {
      template: "Mortal",
      concept: "x",
      attributes: {},
      skills: {},
      specialties: {},
    } as unknown as CofdCgState["sheet"],
    isSubmitted: true,
    isApproved: false,
    submittedJob: num,
    submittedAt: Date.now(),
  };
}

async function seedJob(num: number, submittedBy: string): Promise<IJob> {
  const job: IJob = {
    id: `job-${num}`,
    number: num,
    title: `Chargen ${num}`,
    bucket: "CGEN",
    status: "new",
    submittedBy,
    submitterName: "Test",
    description: "x",
    comments: [],
    createdAt: Date.now(),
    updatedAt: Date.now(),
  };
  await jobs.create(job);
  return job;
}

async function seedPlayer(
  target: ReturnType<typeof mockPlayer>,
): Promise<void> {
  await dbojs.delete({ id: target.id }).catch(() => {});
  await dbojs.create({
    id: target.id,
    flags: [...target.flags].join(" "),
    data: { name: target.name, ...target.state },
  });
}

describe("mail integration", OPTS, () => {
  it("sendCofdMail inserts a well-formed message", async () => {
    const to = `mail-unit-${crypto.randomUUID()}`;
    await sendCofdMail({
      to,
      subject: "Test subject",
      body: "Test body.",
    });
    const all = await mailDb.find({});
    const inbox = all.filter(
      (m) => Array.isArray(m.to) && m.to.includes(to),
    );
    assertEquals(inbox.length, 1);
    assertEquals(inbox[0].from, "#0");
    assertEquals(inbox[0].subject, "Test subject");
    assertEquals(inbox[0].message, "Test body.");
    assertEquals(inbox[0].read, false);
    assertEquals(inbox[0].folder, "inbox");
  });

  it("approve calls the mail path (visible in player notification)", async () => {
    const num = 7001;
    const tid = `tgt-approve-${crypto.randomUUID()}`;
    const target = mockPlayer({
      id: tid,
      name: "Eve",
      state: { cofd_cg: fakeCgState(num) },
    });
    await seedJob(num, tid);
    await seedPlayer(target);

    const u = mockU({
      me: mockPlayer({ id: "wiz", name: "Wiz" }),
      args: ["", "Eve=Welcome."],
    });
    u.util.target = () => Promise.resolve(target);
    u.util.displayName = (o) => o.name ?? "?";

    await approveExec(u);
    assertStringIncludes(u._sent.join("\n"), "Character Approved");
    const mails = await mailDb.find({});
    assertEquals(
      mails.some(
        (m) =>
          Array.isArray(m.to) &&
          m.to.includes(tid) &&
          String(m.subject).toLowerCase().includes("approved"),
      ),
      true,
    );
  });

  it("deny calls the mail path (visible in player notification)", async () => {
    const num = 7002;
    const tid = `tgt-deny-${crypto.randomUUID()}`;
    const target = mockPlayer({
      id: tid,
      name: "Frank",
      state: { cofd_cg: fakeCgState(num) },
    });
    await seedJob(num, tid);
    await seedPlayer(target);

    const u = mockU({
      me: mockPlayer({ id: "wiz", name: "Wiz" }),
      args: ["", "Frank=Too thin."],
    });
    u.util.target = () => Promise.resolve(target);
    u.util.displayName = (o) => o.name ?? "?";

    await denyExec(u);
    assertStringIncludes(
      u._sent.join("\n"),
      "returned for revision by Wiz",
    );
    assertStringIncludes(u._sent.join("\n"), "Too thin.");
  });

  it("approve works with no job at all", async () => {
    const tid = `tgt-approve-noj-${crypto.randomUUID()}`;
    const target = mockPlayer({
      id: tid,
      name: "Gina",
      state: {
        cofd_cg: {
          stage: 6,
          sheet: {
            template: "Mortal",
            concept: "x",
            attributes: {},
            skills: {},
            specialties: {},
          },
          isSubmitted: false,
          isApproved: false,
        },
      },
    });
    await seedPlayer(target);
    const u = mockU({
      me: mockPlayer({ id: "wiz", name: "Wiz" }),
      args: ["", "Gina"],
    });
    u.util.target = () => Promise.resolve(target);
    u.util.displayName = (o) => o.name ?? "?";

    await approveExec(u);
    assertStringIncludes(u._sent.join("\n"), "Character Approved");
    const mails = await mailDb.find({});
    assertEquals(
      mails.some(
        (m) =>
          Array.isArray(m.to) &&
          m.to.includes(tid) &&
          String(m.subject).toLowerCase().includes("approved"),
      ),
      true,
    );
  });
});
