...
export class DenoKvAdapter<T extends WithId> implements IDatabase<T> {
  ...
  async atomicModify(id: string, transform: (current: T) => T, retries = 3): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      if (entry.value === null) throw new Error(`[DenoKvAdapter] Record not found: ${id}`);
    ...
    throw new Error(`[DenoKvAdapter] atomicModify failed after ${retries + 1} attempts on "${id}"`);
  }
  ...
  static async close(): Promise<void> {
    if (DenoKvAdapter.kv) {
      await DenoKvAdapter.kv.close();
      DenoKvAdapter.kv = null;
    }
  }
}
