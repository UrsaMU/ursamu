...
Deno.test("validateLangDef — rejects markov mode in v1", () => {
  const def = {
    schema: 1, name: "x", mode: "markov",
    onsets: [], nuclei: ["a"], codas: [], syllablePatterns: ["V"], wordLenWeights: [1],
  };
...
Deno.test("validateLangDef — rejects empty nuclei", () => {
  const def = {
    schema: 1, name: "x", mode: "phoneme",
    onsets: ["k"], nuclei: [], codas: [""],
    syllablePatterns: ["V"], wordLenWeights: [1],
  };
  assert(!validateLangDef(def, "x").ok);
});
...
Deno.test("validateLangDef — rejects invalid pattern chars", () => {
  const def = {
    schema: 1, name: "x", mode: "phoneme",
    onsets: ["k"], nuclei: ["a"], codas: [""],
    syllablePatterns: ["CVX"], wordLenWeights: [1],
  };
  assert(!validateLangDef(def, "x").ok);
});
...
Deno.test("validateLangDef — rejects all-zero weights", () => {
  const def = {
    schema: 1, name: "x", mode: "phoneme",
    onsets: ["k"], nuclei: ["a"], codas: [""],
    syllablePatterns: ["CV"], wordLenWeights: [0, 0, 0],
  };
  assert(!validateLangDef(def, "x").ok);
});
