...
Examples:
  ...
  exec: (u: IUrsamuSDK) => {
    ...
    if (sw === "garble") return cmdGarble(u, arg);
    return cmdShow(u);
  },
});
...
export async function cmdShow(u: IUrsamuSDK): Promise<void> {
  ...
  const lines = entries
    ...
    .map(
      ([name, skill]) =>
        `  ${u.util.ljust(name, 20)} ${u.util.rjust(String(skill), 3)}%`,
    );
...
export function cmdList(u: IUrsamuSDK): void {
  const all = listLangs();
  if (all.length === 0) {
    u.send("No languages configured.");
    return;
  }
  const lines = all.map(
    (l) => `  ${u.util.ljust(l.name, 20)} ${l.description ?? ""}`,
  );
  u.send(`${HEADER} (configured)\n${lines.join("\n")}`);
}

export async function cmdReload(u: IUrsamuSDK): Promise<void> {
  if (!u.me.flags.has("wizard")) {
  ...
  const msg =
    `Loaded ${report.loaded.length} language(s); re-baked say/pose scripts.` +
...
export async function cmdLearn(u: IUrsamuSDK, arg: string): Promise<void> {
  ...
  const m = arg.match(/^(.+?)=(.+?)\/(-?\d+)$/);
  if (!m) {
    u.send("Usage: +language/learn <player>=<language>/<0-100>");
    return;
  }
  ...
  if (!target || !target.flags.has("player")) {
    u.send(`No such player: ${playerName}`);
    return;
  }
  if (!getLang(langName.trim())) {
    u.send(`Unknown language: ${langName}`);
    return;
  }
  const skill = clampSkill(Number(nStr));
  await setSkill(u, target as IDBObj, langName.trim(), skill);
  u.send(
    `Set ${target.name}'s ${langName.trim().toLowerCase()} skill to ${skill}.`,
  );
}

export function cmdGarble(u: IUrsamuSDK, arg: string): void {
  const m = arg.match(/^(.+?)=(.+?)(?:\/(\d+))?$/);
  if (!m) {
    u.send("Usage: +language/garble <language>=<text>[/<0-100>]");
    return;
  }
  const [, langName, text, skillStr] = m;
  const def = getLang(langName.trim());
  if (!def) {
    u.send(`Unknown language: ${langName}`);
    return;
  }
  const skill = skillStr !== undefined ? clampSkill(Number(skillStr)) : 0;
