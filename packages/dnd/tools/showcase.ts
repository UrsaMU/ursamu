...
// Showcase runner -- executes commands in-process against the shim SDK.
...
function buildMockSDK(player: IDBObj, cmdName: string, args: (string | undefined)[], output: string[], allObjs: IDBObj[], roomCfg?: ShowcaseSetup["room"], dynamic?: IDBObj[]): IUrsamuSDK {
  ...
  const roomState: Record<string, unknown> = { ...MOCK_ROOM.state };
  ...
  return {
    ...
    util: {
      ...
      displayName: (o: IDBObj) => (o.name ?? o.id).split(";")[0],
      ljust: (s: string, w: number) => {
        const clean = s.replace(/%c[a-z]|%[rtnb]/g, "").replace(/\x1b\[[0-9;]*m/g, "");
        const pad = w - clean.length;
        return pad > 0 ? s + " ".repeat(pad) : s;
      },
      rjust: (s: string, w: number) => {
        const clean = s.replace(/%c[a-z]|%[rtnb]/g, "").replace(/\x1b\[[0-9;]*m/g, "");
        const pad = w - clean.length;
    ...
    trigger: (target: string, attr: string, args?: string[]) => Promise.resolve(),
    eval: (targetId: string, attr: string, args?: string[]) => Promise.resolve(""),
...
async function ensureLoaded(objs: IDBObj[]) {
  ...
  if (!_loaded) {
    _loaded = true;
    await shim.loadDefaultCommands();
    const pluginMod = await import("../index.ts");
    if (pluginMod.plugin?.init) {
      await pluginMod.plugin.init();
    }
  }
  _allObjs = objs;
...
function allObjs(state: RunState): IDBObj[] {
  return [state.player, state.admin, ...state.targets.values(), ...state.dynamic, MOCK_ROOM];
}
...
async function renderStep(step: ShowcaseStep, vars: Record<string, string>, state: RunState): Promise<void> {
  ...
  if (step.expect != null) {
    ...
    const cleanLastOut = lastOut.replace(/\x1b\[[0-9;]*m/g, "");
    const ok = cleanLastOut.toLowerCase().includes(want.toLowerCase());
    const tag = ok ? " [OK]" : " [FAIL]";
    console.log("  " + DIM + "expect -> " + want + RESET + tag);
    if (!ok) failures.push({ key: currentKey, step: "expect", expect: want, actual: cleanLastOut.slice(0, 240) });
    return;
  }
  if (step.cmd    != null) {
...
async function pickInteractive(files: ShowcaseFile[]): Promise<ShowcaseFile | null> {
  ...
  const draw = (first: boolean) => {
    ...
    console.log(BOLD + CYAN + "  Showcases" + RESET + DIM + "  -- up/down navigate  Enter select  q quit" + RESET);
...
async function main(): Promise<void> {
  ...
  if (args.help) { console.log("Usage: deno task showcase [key] [--list] [--all]"); return; }
