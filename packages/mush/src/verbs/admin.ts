...
export async function execBoot(u: IUrsamuSDK): Promise<void> {
  ...
  await log("warn", "ADMIN_BOOT", { actor: u.me.id, target: tar.id });
...
export async function execResetToken(u: IUrsamuSDK): Promise<void> {
  ...
  await log("warn", "ADMIN_RESETTOKEN", { actor: u.me.id, target: tar.id });
  u.send(`Reset token for ${u.util.displayName(tar, u.me)}: ${token} (expires in 1 hour)`);
}

const ALLOWED_SITE_CONFIGS = new Set([
  "server.name", "server.motd", "server.description", "server.banner",
...
export async function execSite(u: IUrsamuSDK): Promise<void> {
  ...
  if (!ALLOWED_SITE_CONFIGS.has(setting)) {
    await log("warn", "ADMIN_SITE_BLOCKED", { actor: u.me.id, setting, value });
    u.send(`Unknown or protected config key: ${setting}. Allowed: ${[...ALLOWED_SITE_CONFIGS].join(", ")}`);
    return;
  }
  await log("warn", "ADMIN_SITE_SET", { actor: u.me.id, setting, value });
...
export async function execShutdown(u: IUrsamuSDK): Promise<void> {
  if (!isAdmin(u)) { u.send("Permission denied."); return; }
  u.here.broadcast(`%chGame>%cn Server @shutdown initiated by %ch${u.me.name || u.me.id}%cn.`);
