...
const playerTags = new DBO<IPlayerTag>("mush.playerTags");
const serverTags = new DBO<IServerTag>("mush.serverTags");
...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (eqIdx === -1) {
      ...
      const obj = results[0];
      u.send(
        `Tag '${name}' → ${obj ? `${obj.name}(#${existing.objectId})` : `#${existing.objectId} (not found)`}`,
      );
      return;
    }
...
Examples:
  ...
  exec: async (u: IUrsamuSDK) => {
    ...
    if (existing) {
      await serverTags.update(
        { id: tagName },
        { ...existing, objectId: target.id, setterId: u.me.id },
      );
