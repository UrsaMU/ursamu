...
export async function execConnect(u: IUrsamuSDK): Promise<void> {
  ...
  if (failedAttempts && failedAttempts > 0) {
    u.send(
      `%ch%cr${failedAttempts} failed login attempt${failedAttempts === 1 ? "" : "s"} since your last visit.%cn`,
    );
  }
...
export async function execPassword(u: IUrsamuSDK): Promise<void> {
  ...
  const isStaff =
    actor.flags.has("superuser") || actor.flags.has("admin") || actor.flags.has("wizard");
