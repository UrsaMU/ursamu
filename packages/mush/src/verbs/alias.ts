...
export async function execAlias(u: IUrsamuSDK): Promise<void> {
  ...
  if (aliasVal) {
    const takenAlias = await u.db.search({
      "data.alias": new RegExp(`^${escapeRegex(aliasVal)}$`, "i"),
    });
    const takenName = await u.db.search({
      "data.name": new RegExp(`^${escapeRegex(aliasVal)}$`, "i"),
    });
    if (takenAlias.length > 0 && takenAlias[0].id !== tar.id) {
      u.send("That alias is already taken.");
      return;
    }
    if (takenName.length > 0 && takenName[0].id !== tar.id) {
