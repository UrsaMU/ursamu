...
export async function objectsHandler(req: Request, userId: string): Promise<Response> {
  ...
  if (path === "/api/v1/objects" && req.method === "POST") return await createObject(req, userId);
  if (path === "/api/v1/objects" && req.method === "GET")  return await searchObjects(url, userId);
...
async function linkRoute(req: Request, exit: IDBOBJ): Promise<Response> {
