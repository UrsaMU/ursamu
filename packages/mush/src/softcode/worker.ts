 ...
 *
 * This file runs inside a Web Worker — NO imports from the main package.
 * All SDK types are declared inline.
 */
interface IDBObj {
  id: string;
  name?: string;
...
interface IMail {
  ...
  body?: string; read?: boolean; timestamp?: number;
}
...
interface IUrsamuSDK {
  ...
  eval(targetStr: string, attr: string, args?: string[]): Promise<string>;
...
self.onmessage = async (e: MessageEvent) => {
  ...
  const u: IUrsamuSDK = {
    ...
    util: {
      ...
      sprintf,
      stripSubs,
      parseDesc: (desc: string, actor: IDBObj, tgt: IDBObj) => {
        const ser = (o: IDBObj & { broadcast?: unknown }) => {
          const { broadcast: _b, ...rest } = o;
          void _b;
          return rest;
        };
        return request<string>("util:parseDesc", { desc, actor: ser(actor), target: ser(tgt) });
      },
      resolveFormat: (tgt: IDBObj, slot: string, defaultArg: string) =>
      ...
      resolveFormatOr: (tgt: IDBObj, slot: string, defaultArg: string, fallback: string) =>
    ...
    db: {
      ...
      create: (tmpl: Partial<IDBObj>) => request<IDBObj>("db:create", { template: tmpl }),
